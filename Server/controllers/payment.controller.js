...
const createPayment = catchAsync(async (req, res, next) => {
    ...
    if (Number(amount) !== Number(event.price)) {
        throw new AppError("Amount is not the same as the service required amount", 400);
    }
    const currency = "ETB";
    ...
    try {
        const payment = await axios.post(
            CHAPA_CREATE_PAYMENT,
            {
                ...
                email,
                tx_ref,
                callback_url: "https://velvetshebook.com/api/chapa-webhook",
        ...
        console.error("Chapa API Error Details:", error.response?.data || error.message);
