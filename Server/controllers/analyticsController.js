...
export const get_dashboard_stats = async (req, res) => {
  try {
    ...
    const pendingApprovals = await UserTicketModel.countDocuments({ status: "pending" });
    ...
    const totalEarnings = revenueAggr.length > 0 ? revenueAggr[0].totalEarnings : 0;
    const recentEarnings = recentRevenueAggr.length > 0 ? recentRevenueAggr[0].recentEarnings : 0;
...
export const get_transaction_ledger = async (req, res) => {
  try {
    ...
    const formattedTransactions = transactions.map(item => ({
      ...item.toObject(),
      commissionAmount: item.status === 'paid' ? (item.totalAmount * 0.10) : 0 
    }));
