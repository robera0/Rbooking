...
import { Check, ChevronDown, Heart, Share2, MapPin, Star, Ticket, Trash2, ArrowRight } from "lucide-react";
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-transparent">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-500">Syncing Collection</span>
        </div>
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16 px-4">
        <div className="space-y-4">
          ...
          <h1 className="text-4xl md:text-6xl font-black uppercase italic tracking-tighter text-white leading-none">
            Saved <span className="text-transparent bg-clip-text bg-gradient-to-r from-white to-white/20">Events</span>
          </h1>
        </div>
            ...
            {sortedEvents.map((w, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* IMAGE SECTION */}
                  <Link 
                    to={w?.tickets?.length > 0 ? `/events/${w._id}/tickets/${w.tickets[0]._id}` : `/events/${w._id}`}
                  ...
                  <div className="flex-1 p-8 md:p-10 flex flex-col justify-between relative z-10">
                    <div className="space-y-6">
                      <div className="flex items-center justify-between">
                         <div className="flex items-center gap-1.5 text-amber-400">
                           {[...Array(5)].map((_, i) => (
                             <Star key={i} size={10} fill="currentColor" className="opacity-80" />
                           ))}
                           <span className="text-[10px] font-black ml-1 text-gray-500">{w?.rating?.score || "4.8"}</span>
                         </div>
                         <div className="flex items-center gap-2 text-gray-600">
                            <MapPin size={12} strokeWidth={3} />
                            <span className="text-[10px] font-black uppercase tracking-widest">{w?.locale || "Main Venue"}</span>
                         </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <p className="text-gray-500 text-xs font-medium leading-relaxed max-w-md line-clamp-2">
                           Experience the pulse of the city at this exclusive gathering. Secured by Paysso Interactive.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="mt-10 pt-8 border-t border-white/[0.04] flex items-center justify-between gap-6">
                      <div className="space-y-1">
                        <p className="text-[9px] font-black text-gray-600 uppercase tracking-widest">Starting From</p>
                        <p className="text-2xl font-black italic text-white leading-none">
                          {w?.priceRanges?.[0]?.min || "0"}
                          <span className="text-[#FF7A00] ml-1 text-sm tracking-normal capitalize">ETB</span>
                        </p>
                      </div>
