import { AvailableHrs } from "./HourMenu";
...
import { Duration } from "./AddServiceMenu";
...
const TimeInput = ({ value, action }) => {
  return <input value={value} type="time" onChange={action} />;
};
const Hour = () => {
  const days = [
    "Sunday",
  ...
  const [defaultStarthour, setDefaultStartHour] = useState("8:00 AM");
  const [defaultEndhour, setDefaultEndHour] = useState("5:00 PM");
          ...
          {days.map((day, index) => (
            <>
              ...
              {timeInfo == index && (
                <>
                  <div>
                    <div className="absolute top-16  left-95 pl-4 w-[100%] h-72 bg-[#202020] flex flex-col justify-around rounded-lg">
                      <div className=" w-[90%] flex h-12 bg-[#343434] hover:bg-[#323232] cursor-pointer rounded-sm">
                        <Toggle
                          action={() => setToggleOn((prev) => !prev)}
                          toggleOn={toggleOn}
                          name="Avialable"
                        />
                      </div>
                      {toggleOn && (
                        <>
                          <div className=" w-[90%] h-12  bg-[#343434] rounded">
                            <div className="  flex justify-between pl-4 h-12  border-b  hover:bg-[#323232]">
                              {sidetime ? (
                                /* make able to add time values */ <>
                 ...
                 w-full h-12 rounded cursor-pointer
                 px-3 text-[#168FF4] font-light"
                                        >
                                          <span>
                                            {defaultStarthour} -{" "}
                                            {defaultEndhour}
                                          </span>
                                          <Trash2 className="w-5 h-5 mr-3 text-red-500 hover:text-red-800" />
                                        </button>
                                      </>
                                    )}
                                  </div>
        ...
        <div className=" w-full space-y-6">
          ...
          <div className="space-y-4 pl-4">
            <div className="group w-[90%] h-12  flex items-center  justify-between bg-[#343434]  hover:bg-[#323232] rounded">
              <label className="pl-4 text-white font-light" htmlFor="button">
                Date-Specefic Hours
              </label>
              <button
                onClick={() => setDateSpecefic((prev) => !prev)}
                className={` ${
                  dateSpecefic && "rotate-90"
                } pl-4 h-full text-white font-light cursor-pointer outline-none transition ease-in-out duration-300  `}
              >
          ...
          <div className="relative pl-4">
            ...
            <p className="text-[#8C8484]">
              set how far advanced appointment must be booked eg , at least
              1hour before
            </p>
            {minhr && (
          ...
          <div className="relative pl-4">
            <div className="group w-[90%] h-12 flex items-center  justify-between bg-[#343434]  hover:bg-[#323232] rounded">
              ...
              <button
                onClick={() => setmaxBokking((prev) => !prev)}
                className={` ${
                  maxbooking && "rotate-360"
                } pl-4 h-full text-white font-light cursor-pointer outline-none transition ease-in-out duration-300  `}
              >
                <ChevronRight className="text-sm mr-3 text-[#168FF4]" />
              </button>
            ...
            <p className="text-[#8C8484]">
              define how far into the future booking can be made such as up to 2
              months in advance
            </p>
            {maxbooking && (
              <div className="absolute fixed top-90 left-122  h-32 inset-0 z-50 ">
          ...
          <div className="flex mb-12 justify-center items-center">
            <button
              ...
              className=" w-56 h-12 text-center text-white font-semibold bg-[#343434] cursor-pointer rounded"
            >
              {" "}
              Done{" "}
            </button>
